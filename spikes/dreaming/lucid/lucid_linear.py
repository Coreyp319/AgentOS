...
def start(session, opening_image, private=False, _trusted_seed=False):
    # _trusted_seed: the opening was generated by us (an abstract frame), not a user upload, so the
    # B2 real-person likeness guard doesn't apply. User-supplied seeds still pass the guard.
    if not _trusted_seed and not seed_image_guard(opening_image):
