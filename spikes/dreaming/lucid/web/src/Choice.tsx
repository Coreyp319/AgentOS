...
// next-segment length options (frames @16fps -> seconds). Mirrors lucid_engine MIN_LEN/MAX_LEN and
// the 4k+1 latent stride; the engine clamps anything off-list, so this list is purely the UI offer.
...
export default function Choice({ state }: { state: LucidState }) {
  ...
  async function fire(prompt: string, label: string) {
    try {
      const j = await dream.mutateAsync({ prompt, label, length: len })
        ...
        <>
          <div className="seglen">
            ...
            <div className="seglen-opts" role="group" aria-label="Next segment length">
              {LENGTHS.map((o) => (
                <button
                  key={o.f} type="button" className={'lenbtn' + (len === o.f ? ' on' : '')}
                  aria-pressed={len === o.f} disabled={busy} onClick={() => setLen(o.f)}
                >{o.s}</button>
              ))}
            </div>
          </div>
          ...
          />
          {/* acknowledge the pick the instant it's fired — the state poll can be up to ~2.5s behind, and a
              minutes-long commitment shouldn't sit on a frozen, dimmed menu with no feedback */}
