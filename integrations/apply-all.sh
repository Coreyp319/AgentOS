...
#   * Lucid dream-loop surface  (http://127.0.0.1:8765) — interactive dreaming (ADR-0015)
...
systemctl --user --no-pager --no-legend list-units \
  hermes-gateway.service hermes-dashboard.service comfyui.service agentos-lucid.service agentos-status-panel.service 2>/dev/null \
  | sed 's/^/  /' || true
